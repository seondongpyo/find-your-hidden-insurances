...
class UserControllerTest {
    ...
    private static final AuthUser AUTH_USER = new AuthUser(1L, "유저", "user", "1234", Role.USER);
	private static final User USER = new User("유저", "user", "1234", Role.USER);
    ...
    void detail() throws Exception {
        mvc.perform(get("/user/detail")
                .with(user(AUTH_USER)))
            .andExpect(status().isOk())
			.andExpect(model().attribute("user", new UserResponseDTO(USER)))
